...
function readStoredJSON(key,fallback){
 try{
   ...
   console.error(`Donnée locale illisible : ${key}`,error);
   // On conserve la valeur défectueuse pour permettre un diagnostic/export,
   // mais l'application redémarre avec une valeur saine au lieu de planter.
   return structuredClone(fallback);
 }
}
function objectOr(value,fallback={}){return value&&typeof value==="object"&&!Array.isArray(value)?value:structuredClone(fallback)}
function arrayOr(value,fallback=[]){return Array.isArray(value)?value:structuredClone(fallback)}

let products=arrayOr(readStoredJSON("bz_products",window.SEED_PRODUCTS),window.SEED_PRODUCTS)
 .filter(p=>p&&typeof p==="object"&&typeof p.name==="string");
products.forEach(p=>{if(["œuf","œufs","oeufs"].includes(normalize(p.name)))p.name="Oeuf"});
let recipes=arrayOr(readStoredJSON("bz_recipes",window.SEED_RECIPES),window.SEED_RECIPES)
...
let history=arrayOr(readStoredJSON("bz_history",[]))
 ...
 .map(course=>({...course,date:course.date||new Date(0).toISOString(),products:Array.isArray(course.products)?course.products.filter(x=>typeof x==="string"):[],amount:Number(course.amount)>0?Number(course.amount):null}));
let streaks=objectOr(readStoredJSON("bz_streaks",{}));
let options=objectOr(readStoredJSON("bz_options",{"reminders":false,"suggestions":true,"autoFavorites":true}),{"reminders":false,"suggestions":true,"autoFavorites":true});options.seasonal=true;
...
let aisles=arrayOr(readStoredJSON("bz_aisles",null),["Entrée","Fruits & légumes","Charcuterie","Boucherie","Produits frais","Épicerie","Eau et sodas","Surgelés","Hygiène","Entretien","Sortie"]);
if(!aisles.length)aisles=["Entrée","Fruits & légumes","Charcuterie","Boucherie","Produits frais","Épicerie","Eau et sodas","Surgelés","Hygiène","Entretien","Sortie"];
let stores=arrayOr(readStoredJSON("bz_stores",[])).filter(store=>store&&typeof store==="object").map(store=>({...store,cells:Array.isArray(store.cells)?store.cells:[],route:Array.isArray(store.route)?store.route:[]}));
...
let profile=objectOr(readStoredJSON("bz_profile",{"name":"Benjamin","image":""}),{"name":"Benjamin","image":""});
...
let v472SuggestionOffsets=objectOr(readStoredJSON("bz_home_suggestion_offsets",{"Ptit dej":0,"Plat":0,"Entrée":0,"Dessert":0}),{"Ptit dej":0,"Plat":0,"Entrée":0,"Dessert":0});
...
renderInverseRecipes=function(){
  ...
  .filter(item=>category==="all"||(item.recipe.category||"Plat")===category)
  .sort((a,b)=>{
    // Classe d'abord selon la part réelle de la recette déjà disponible.
    // Exemple : 4/4 (100 %) doit passer avant 5/10 (50 %).
    const percentA=a.relevant.length ? a.present.length/a.relevant.length : 1;
    const percentB=b.relevant.length ? b.present.length/b.relevant.length : 1;
...
// V5.1.3 data resilience + recipe stock highlight
(function(){
...
(function installV611Features(){
  ...
  function addIngredientToShopping(name, silent=false){
    const product=productByName(name)||ensureProduct(name);
    const existing=Object.keys(shopping||{}).find(item=>productKey(item)===productKey(product.name));
    if(existing){
      if(!silent)showActionMessage(`${product.name} est déjà dans la liste`);
      return false;
    }
    shopping[product.name]=true;
    delete bought[product.name];
    save();
    if(!silent)showActionMessage(`${product.name} ajouté à la liste`);
    return true;
  }
  ...
  renderInverseRecipes=function(){
    ...
    const ranked=recipes.map(recipe=>{
      ...
      for(const name of (recipe.ingredients||[])){
        const key=productKey(name);if(!key||seen.has(key))continue;seen.add(key);unique.push({name,key});
      }
      const present=unique.filter(item=>available.has(item.key));
      const missing=unique.filter(item=>!available.has(item.key));
      return {recipe,present,missing,total:unique.length,ratio:unique.length?present.length/unique.length:0,touches:!selected.length||selected.some(key=>seen.has(key))};
    }).filter(item=>item.touches)
      .filter(item=>!query||normalize(item.recipe.name).includes(query))
      .filter(item=>category==="all"||(item.recipe.category||"Plat")===category)
  ...
  document.addEventListener("click",event=>{
    const ingredient=event.target.closest("[data-add-ingredient]");
    if(!ingredient)return;
    event.preventDefault();event.stopPropagation();
    window.addRecipeIngredient(ingredient.dataset.addIngredient);
  });

  function deleteHistoryCourse(id){
  ...
  renderHistory=function(){
    $("#historyList").innerHTML=history.length?history.map((course,index)=>{
      ...
      return `<div class="history-swipe-row" data-history-id="${esc(String(course.id))}">
        <div class="history-delete-bg" aria-hidden="true"><span>🗑</span></div>
        <article class="history-card history-swipe-card">
          <div class="history-head"><div><strong>${new Date(course.date).toLocaleDateString("fr-BE",{dateStyle:"long"})}</strong><small> · ${(course.products||[]).length} produits · ${amount}</small></div><span>▾</span></div>
          <div class="history-products">${(course.products||[]).map(name=>`<button class="history-product" onclick="event.stopPropagation();addHistoryProduct('${jsesc(name)}')">+ ${esc(name)}</button>`).join("")}</div>
        </article>
      </div>`;
    }).join(""):'<div class="empty">Aucune course enregistrée</div>';
    save();
  ...
  let swipe=null;
  document.addEventListener("pointerdown",event=>{
    const card=event.target.closest(".history-swipe-card");
    if(!card||event.target.closest("button"))return;
    swipe={card,row:card.closest(".history-swipe-row"),startX:event.clientX,startY:event.clientY,x:0,moved:false};
